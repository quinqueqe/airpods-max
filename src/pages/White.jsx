import React from 'react'

// img hero
import heroImg from '../assets/img/big/airpods-max-select-silver-202011 1.png'

// img mini
import blackImg from '../assets/img/mini/black.png'
import redImg from '../assets/img/mini/red.png'
import greenImg from '../assets/img/mini/green.png'
import blueImg from '../assets/img/mini/blue.png'

// img extremely
import extremelyImg from '../assets/img/leftvers/white.png'

// img preserves
import preservesImg from '../assets/img/back/white.png'

import Header from '../components/Header'
import Hero from '../components/Hero'
import Choose from '../components/Choose'
import Extremely from '../components/Extremely'
import Preserves from '../components/Preserves'


const White = () => {
		return (
			<>
				<div className='background-white'>
					<Header />
					<Hero img={heroImg} className='mt52'/>
				</div>
				<Choose img1={greenImg} img2={blueImg} img3={redImg} img4={blackImg} 
				link1='/airpods-max/green'
				link2='/airpods-max/blue'
				link3='/airpods-max/'
				link4='/airpods-max/black'/>
				<Extremely img={extremelyImg} />
				<Preserves img={preservesImg} />
			</>
		)

}


export default White